<template>
  <div class="CalendarFormCommentsModal">
    <b-modal visible
             title="Comentários"
             body-class="CalendarFormCommentsModal--modal-body"
             ...
             @ok="$emit('onClose')">
      <div>
        <div v-for="(comment, index) in propComments" :key="`comment_${_uid}_${index}`">
          <div class="col">
              ...
              <div class="row">
                <div class="col-2">
                  <div class="row">
                    <div class="col">
                      <b-avatar
                        :src="comment.user.image || require('@/assets/images/avatars/user.png')" />
                    </div>
                  </div>
                  <div class="row">
                    <div class="col">
                ...
                <div class="col">
                  <div class="text-black-50">
                    <b><small>{{ comment.date | invertDate }} - {{ comment.time }}</small></b>
                  </div>
                  <div v-html="convertTextLink(comment.comment)" />
                </div>
              </div>
            </b-card>
          </div>
...
import { convertTextLink } from '@/utils/helpers'

export default {
  name: "CalendarFormCommentsModal",
  props: {
    propComments: {
      type: Array,
      default: () => ([])
  ...
  methods: {
    async init () {
      try {
        loader.show()
        const data = await TaskCommentsService.index(this.propTask._id)
